"""Mirrors each project's (and each standalone note base's) current
tasks/knowledge into its own repo on archeion (the agorae deployment's Forgejo) — a second, independent place
...
def kb_repo_name(kb_slug: str) -> str:
    """Note bases share the org with projects, so their repo names are
    prefixed to keep a project and a note base with the same slug apart."""
    return f"kb-{kb_slug}"
...
def sync_project(slug: str, name: str, content_dir: Path, acting_user: str, message: str) -> None:
    """Copies tasks/ and knowledge/'s current state into this scope's
    mirror repo (`slug` is the repo name — see kb_repo_name for note bases) and pushes one new commit, if anything actually changed."""
    if not _configured():
        return
    try:
        ...
        for sub in ("tasks", "knowledge"):
            ...
            if src_sub.exists():
                # a submodule folder's checkout carries its own .git link —
                # copying that in would make the mirror treat it as a nested repo
                shutil.copytree(src_sub, dest_sub, ignore=shutil.ignore_patterns(".git"))
        repo.git.add(A=True)
        ...
        print(f"archeion mirror: sync failed for {slug}: {err}")
...
def sync_collaborators(slug: str, name: str, members: list[str], get_email) -> None:
    ...
    try:
        ...
        # one at a time, each on its own: a member with no archeion account
        # yet (404) must not stop everyone else from getting access
        for login in sorted(target - current):
            try:
                _admin_api(
                    "PUT", f"/repos/{ARCHEION_ORG}/{slug}/collaborators/{login}", json={"permission": "read"}
                ).raise_for_status()
            except httpx.HTTPError as err:
                print(f"archeion mirror: couldn't add {login} to {slug}: {err}")
        for login in sorted(current - target):
            try:
                _admin_api("DELETE", f"/repos/{ARCHEION_ORG}/{slug}/collaborators/{login}").raise_for_status()
            except httpx.HTTPError as err:
                print(f"archeion mirror: couldn't remove {login} from {slug}: {err}")
    except httpx.HTTPError as err:
        print(f"archeion mirror: collaborator sync failed for {slug}: {err}")
